...
struct Hash_Item {
	...
	//Get Key Value
	string get_key() {
		string a = key.get_WrapperString();
		return a;
	}
	...
	void Delete_(k data) {
		...
		for (int i = 0; i < capacity; i++) {
			...
			if (arr[index].nature == 1) {
				...
				string num = arr[index].get_key();
				string num2 = a.get_key();
...
struct WrapperString {
	...
	long long hashcode(){
		...
		for (int i = 0; i < StringVal.length(); i++) {
			hash_val= (hash_val+ (StringVal[i] - 'a' + 1) * power_of_p)% m;
	...
	bool operator==(WrapperString& obj) {
		if (obj.StringVal == this->StringVal) { return true; }
		else { return false; }
	}

	//Assigning Value
	void operator=(WrapperString& x) {
		this->StringVal = x.StringVal;
	}

	//Ostream Printing
	friend ostream& operator<<(ostream& out, const WrapperString& x) {
		out << x.StringVal;
		return out;
	}
};
...
int main() {
	...
	Hash_Map<WrapperString, float> hash(4);

	cout << "Inserting These Elements" << endl;

	WrapperString w1("20L-2072");
	WrapperString w2("20L-2076");
	WrapperString w3("20L-2068");
	WrapperString w4("20L-0000");
	...
	cout << "Finding Key '20L-2072' " << endl;

	if (hash.Search_hashkey(w1)) {
		cout << "Key Found" << endl;
	}
	else {
